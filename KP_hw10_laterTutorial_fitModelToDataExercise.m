...
% EXERCISE:
deltaS = 1/(std(1./RTs));
muR = mean(1./RTs) * deltaS;
initialValues = [deltaS, muR];
...
[param, nllk] = run(gs, problem);
later_plotReciprobit(RTs, param, gca, [], 'b')
...
figure;
subplot(2,1,1);
histogram(RTs); % Adjust 'BinWidth' to set the bin size
xlabel('Reaction Time (s)');
ylabel('Frequency');
title('Histogram of RT');

subplot(2,1,2);
histogram(1./RTs); % Adjust 'BinWidth' to set the bin size
xlabel('1/Reaction Time (s)');
ylabel('Frequency');
title('Histogram of 1/RT');
